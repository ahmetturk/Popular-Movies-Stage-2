...
public class FetchVideosTask extends AsyncTask<String, Void, List<Video>> {
    ...
    private final AsyncTaskCompleteListener<List<Video>> listener;

    public FetchVideosTask(AsyncTaskCompleteListener<List<Video>> listener) {
    ...
    private List<Video> getVideosDataFromJson(String videosJsonStr) throws JSONException {
        JSONObject jsonObject = new JSONObject(videosJsonStr);
        JSONArray videos = jsonObject.getJSONArray("results");

        List<Video> videosList = new ArrayList<>(videos.length());
